...
do
    ...
    for j in {0..6}
    do
        query='db.A.find({A1 : {$lte : 50}}, {A1: true, A2: true, _id: false});'
        mongo test --eval "
            DBQuery.shellBatchSize = ${BatchLimit};
    ...
    for j in {0..2}
    do
        query = 'db.B.aggregate([{$sort: {B3 : 1} },{$project: {_id: false, B1: true, B2: true, B3: true}}],{ "allowDiskUse" : true });'
        mongo test --eval "
            DBQuery.shellBatchSize = ${BatchLimit};
            db.setProfilingLevel(2);
            ${query};            
    ...
    for j in {0..2}
    do
        query='db.B.aggregate([{$group: {_id: "$B2", total: {$sum: 1}}},{$group: {_id: null, answer: {$avg: "$total"}}},{$project: {_id: false,answer: true}}]);'
        mongo test --eval "
            DBQuery.shellBatchSize = ${BatchLimit};
            db.setProfilingLevel(2);
            ${query};
    ...
    for j in {0..2}
    do
        query='db.B.aggregate([{$lookup: {from: "A",localField: "B2",foreignField: "A1",as: "temp"}},{$project:{_id: false,B1: true,B2: true,B3: true,A2: {$arrayElemAt: ["$temp.A2", 0]},}}])'
        mongo test --eval "
            DBQuery.shellBatchSize = ${BatchLimit};
            db.setProfilingLevel(2);
            ${query};
