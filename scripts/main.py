...
#Plotting Execution times per Query
xval = [i+1 for i in range(9)]
for query in range(4):
    sq1 = []
    sq2 = []
    ...
    for dataset in range(9):
        sq1.append(sqlite3[dataset][query][0])
        sq2.append(maria[dataset][query][0])
        sq3.append(mariaidx[dataset][query][0])
        sq4.append(mongo[dataset][query][0])
    plt.title("Query " + str(query+1))
    plt.xlabel("Dataset Number")
    plt.ylabel("Mean Runtime in Seconds")
    plt.plot(xval, sq1, label='Sqlite3', marker='o')
    plt.plot(xval, sq2, label='MariaDB (Without Index)',marker='o')
    ...
    plt.plot(xval, sq4, label='MongoDB',marker='o')
    plt.legend()
    plt.savefig('./Images/Part3/Query'+str(query+1))
    plt.clf()
